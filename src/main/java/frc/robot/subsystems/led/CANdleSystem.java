 ...
 *
 * <p>Copyright (C) Cross The Road Electronics. All rights reserved.
 *
 * <p>Cross The Road Electronics (CTRE) licenses to you the right to use, publish, and distribute
 * copies of CRF (Cross The Road) firmware files (*.crf) and Phoenix Software API Libraries ONLY
 * when in use with CTR Electronics hardware products as well as the FRC roboRIO when in use in FRC
 * Competition.
 *
 * <p>THE SOFTWARE AND DOCUMENTATION ARE PROVIDED "AS IS" WITHOUT WARRANTY OF ANY KIND, EITHER
 * EXPRESS OR IMPLIED, INCLUDING WITHOUT LIMITATION, ANY WARRANTY OF MERCHANTABILITY, FITNESS FOR A
 * PARTICULAR PURPOSE, TITLE AND NON-INFRINGEMENT. IN NO EVENT SHALL CROSS THE ROAD ELECTRONICS BE
 * LIABLE FOR ANY INCIDENTAL, SPECIAL, INDIRECT OR CONSEQUENTIAL DAMAGES, LOST PROFITS OR LOST DATA,
 * COST OF PROCUREMENT OF SUBSTITUTE GOODS, TECHNOLOGY OR SERVICES, ANY CLAIMS BY THIRD PARTIES
 * (INCLUDING BUT NOT LIMITED TO ANY DEFENSE THEREOF), ANY CLAIMS FOR INDEMNITY OR CONTRIBUTION, OR
 * OTHER SIMILAR COSTS, WHETHER ASSERTED ON THE BASIS OF CONTRACT, TORT (INCLUDING NEGLIGENCE),
 * BREACH OF WARRANTY, OR OTHERWISE
 */

/**
 * Description: The CANdle MultiAnimation example demonstrates using multiple animations with
 * CANdle. This example has the robot using a Command Based template to control the CANdle.
 *
 * <p>This example uses: - A CANdle wired on the CAN Bus, with a 5m led strip attached for the extra
 * animatinos.
 *
 * <p>Controls (with Xbox controller): Right Bumper: Increment animation Left Bumper: Decrement
